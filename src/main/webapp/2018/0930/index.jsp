<html>
<%@ page contentType="text/html; charset=utf-8" language="java"%>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
  <link rel="stylesheet" href="/stylesheets/main.css" type="text/css">
  <link rel="stylesheet" href="/jquery-ui-1.11.2/jquery-ui.min.css">
  <script src="/jquery/3.3.1/jquery-3.3.1.min.js"></script>
  <script src="/jquery-ui-1.11.2/jquery-ui.min.js"></script>
  <script type="text/javascript" src="/js/main.js"></script>
</head>

<body>
	<div id="contenedor">
		<div id="header"></div>
    <table class="tabla_contenedor art-article">
			<tr>
				<td class="td_contenedor">
          <table>
            <tr class="top">
              <td>
<%@ include file="/2018/aviso1.jsp"%>
<%@ include file="/2018/bienvenida.jsp"%>
              </td>
              <td>
<%@ include file="/2018/1011/aviso1.jsp"%>
<%@ include file="/2018/0930/proxima.jsp"%>
<%@ include file="/2016/ventajas_socio.jsp"%>
              </td>
              <td>
<%@ include file="/2018/05/aviso1.jsp"%>
<%@ include file="/2016/apunte.jsp"%>
<%@ include file="/2018/seguros/fmm/federacion.jsp"%>
<%@ include file="/2014/aviso.jsp"%>
<%@ include file="/2016/noticias2.jsp"%>
              </td>
            </tr>
          </table>
				</td>
			</tr>
      <tr>
        <td class="td_contenedor">
<%@ include file="/2018/0930/excursiones.jsp"%>
<%@ include file="/2018/0930/salidas.jsp"%>
        </td>
      </tr>
			<tr>
				<td class="td_contenedor">
<%@ include file="/jsp/fotos.jsp"%>
				</td>
			</tr>
		</table>

<%@ include file="/txt/footer.txt"%>
	</div>
</body>
