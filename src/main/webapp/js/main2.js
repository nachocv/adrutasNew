...
var getInit = function() {
	...
	$.ajax({url: "/getInit?fecha=" + fecha + "&pathname=" + window.location.pathname, success: function(result) {
		url = result;
		console.log("url: " + url + ". antes");
		$(".init").load(url);
    console.log("url: " + url + ". despues");
    },
	error : function(jqXHR, status, error) {
		alert('Disculpe, existió un problema');
	}});
...
var putInclude = function() {
  console.log("putInclude. Inicio");
  $(".include" ).each(function() {
    esto = $(this);
    esto.load(esto.attr("src"));
  });
  console.log("putInclude. Final");
}
  ...
  if (datos!=undefined && datos.id_persona!=undefined) {
    ...
    $.ajax({
      ...
      success: function(result) {
        ...
        objects.each(function() {
          var esto = $(this);
          var salida = esto.attr("src");
          console.log("getSalidas salida: " + salida);
          esto.html("<br/><a href='#ex_" + salida + "' rel='modal:open'>Tienes " + resultados["especiales"][salida].length + " puntos</a>");
        });
      },
      error : function(jqXHR, status, error) {
        alert('Disculpe, existió un problema');
...
var getUsuario = function() {
  console.log("putInclude(). url: " + url);
  putInclude();
  $.ajax({
    url: "/getUsuario?pathname=" + (url==undefined? window.location.pathname: url),
    success: function(result) {
      console.log("result: " + result + ". antes");
      comprueba_identificacion(result);
      console.log("result: " + result + ". despues");
    },
    error : function(jqXHR, status, error) {
      alert('Disculpe, existió un problema');
    }
